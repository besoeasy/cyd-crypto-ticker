...
Fetches live prices from CoinGecko and cycles through your configured coins on-screen. Supports up to 8 total slots, with configuration managed through a captive hotspot that can be reopened from the Settings panel.
...
4. Swipe left and right to cycle coins. Tap the middle of a coin screen to open the `SETTINGS` panel. Opening that panel automatically enables the config hotspot, shows the WiFi details on-device, and lets you change all settings from your phone or laptop. Use `BACK` to return.
...
The Settings panel is now a WiFi status page only. It shows the active hotspot details on-device while you configure everything from the captive portal on your phone or laptop. The portal controls the coin list, random coin count, price refresh interval, and auto-rotate interval. Random picks are filled from CoinGecko trending and high-volume markets.
...
Up to **8 coins** are supported. Default timing is a 60 second refresh interval and an 8 second auto-rotate interval, both configurable through the hotspot portal.
